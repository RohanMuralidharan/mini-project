...
3. Create `.env` from `.env.example` and fill your Spotify credentials.
...
1. Create `.env` from `.env.example` and fill your Spotify credentials.

2. Run the small demo script (this may open a browser to complete OAuth):
...
3. The demo writes `saved_tracks_demo.csv` in the current folder and prints a few tracks.
