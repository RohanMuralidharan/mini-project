...
# Prefer local demo CSV if present to avoid requiring Spotify OAuth at runtime
root = Path(__file__).resolve().parent
repo_root = root.parent
csv_path = repo_root / 'saved_tracks_demo.csv'

if csv_path.exists():
    df = pd.read_csv(csv_path)
    # keep compatibility with existing code expecting duration_s column
    ...
    if token:
        ...
        raise RuntimeError("Couldn't get token for that username; create a .env and/or run fetch_demo.py")
